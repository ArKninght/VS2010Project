				 ...
				 char * btsEncKey /*= NULL*/)
{
	...
	if (btsEncKey)	//btsEncKey != NULL
	{
		// 用按位异或解密 pBuff 地址开始的、长iBytesRead的各字节
		LONG i=0, j=0, iLenKey=strlen(btsEncKey);
		for(i=0; i<iBytesRead;i++)
		{
			pBuff[i] = pBuff[i] ^ btsEncKey[j];	// 用按位异或解密
			j++;								// 密钥将使用下一个字节
			if(j>=iLenKey) j=0;					// 密钥使用完一次之后，从头开始继续使用
		}
	}
				 ...
				 char * btsEncKey /*= NULL*/)
{
	...
	if (btsEncKey)	//btsEncKey != NULL
	{
		// 用按位异或加密 pBuff 地址开始的、长iBuffLen的各字节
		LONG i=0, j=0, iLenKey=strlen(btsEncKey);
...
LONG EFPrint( HANDLE hFile, 
			  ...
			  char * btsEncKey /*= NULL */ )
{
	return EFPrint(hFile, stringText.c_str(), styleLineFeed, llWritePos, 
					iShowResume, stringFailInfo.c_str(), btsEncKey);
}
